...
procedure TfSel_EnqIPI.BitBtn1Click(Sender: TObject);
begin
  prc_Consultar;
end;

procedure TfSel_EnqIPI.prc_Consultar;
begin
  cdsEnqIPI.Close;
...
procedure TfSel_EnqIPI.SMDBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfSel_EnqIPI.SMDBGrid1DblClick(Sender: TObject);
begin
  viD_EnqIPI_Pos := cdsEnqIPIID.AsInteger;
  Close;
end;

procedure TfSel_EnqIPI.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := Cafree;
end;

procedure TfSel_EnqIPI.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = 27 then
    Close;
...
procedure TfSel_EnqIPI.FormShow(Sender: TObject);
var
  i : Integer;
begin
...
procedure TfSel_EnqIPI.SMDBGrid1TitleClick(Column: TColumn);
var
  i : Integer;
...
procedure TfSel_EnqIPI.Edit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = Vk_Return then
  begin
