object fContasReceberPagSel: TfContasReceberPagSel
  Left = 128
  Top = 211
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Pagamentos Selecionados'
  ClientHeight = 147
  ClientWidth = 669
  Color = clMoneyGreen
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 55
    Top = 16
    Width = 31
    Height = 13
    Caption = 'Conta:'
  end
  object Label2: TLabel
    Left = 3
    Top = 40
    Width = 83
    Height = 13
    Caption = 'Data Pagamento:'
  end
  object Label12: TLabel
    Left = 9
    Top = 65
    Width = 76
    Height = 13
    Caption = 'Plano de Conta:'
  end
  object SpeedButton8: TSpeedButton
    Left = 332
    Top = 53
    Width = 25
    Height = 25
    Hint = 'Abre o cadastro do plano de contas'
    Glyph.Data = {
      
      8888880BB088888888888800008888888888}
    ParentShowHint = False
    ShowHint = True
    OnClick = SpeedButton8Click
  end
  object Label15: TLabel
    Left = 32
    Top = 90
    Width = 53
    Height = 13
    Caption = 'Sub-Conta:'
  end
  object Label5: TLabel
    Left = 375
    Top = 40
    Width = 252
    Height = 65
    Caption = 
      'Plano Contas:'#13#10'- O sistema vai dar prioridade ao que esta lan'#231'ad' +
      'o'#13#10'  na parcela'#13#10'- Se a parcela n'#227'o possuir plano de contas, o s' +
      'istema'#13#10'  vai gravar o plano que foi informado aqui nesta tela'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clMaroon
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object RxDBLookupCombo1: TRxDBLookupCombo
    Left = 87
    Top = 8
    Width = 313
    Height = 21
    DropDownCount = 8
    LookupField = 'CodConta'
    LookupDisplay = 'NomeConta'
    LookupSource = DM1.dsContas
    TabOrder = 0
  end
  object DateEdit1: TDateEdit
    Left = 87
    Top = 32
    Width = 121
    Height = 21
    NumGlyphs = 2
    TabOrder = 1
  end
  object BitBtn1: TBitBtn
    Left = 192
    Top = 114
    Width = 88
    Height = 31
    Caption = 'Confirmar'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 4
    OnClick = BitBtn1Click
  end
  object BitBtn2: TBitBtn
    Left = 280
    Top = 114
    Width = 88
    Height = 31
    Caption = 'Cancelar'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 5
    OnClick = BitBtn2Click
  end
  object RxDBLookupCombo3: TRxDBLookupCombo
    Left = 87
    Top = 57
    Width = 243
    Height = 21
    DropDownCount = 18
    LookupField = 'Codigo'
    LookupDisplay = 'Descricao'
    LookupSource = DM1.dsPlanoContas
    ParentShowHint = False
    ShowHint = False
    TabOrder = 2
    OnDropDown = RxDBLookupCombo3DropDown
    OnEnter = RxDBLookupCombo3Enter
  end
  object RxDBLookupCombo4: TRxDBLookupCombo
    Left = 87
    Top = 82
    Width = 243
    Height = 21
    DropDownCount = 18
    LookupField = 'CodItem'
    LookupDisplay = 'Nome'
    LookupSource = DM1.dsPlanoContasItens
    ParentShowHint = False
    ShowHint = False
    TabOrder = 3
    OnEnter = RxDBLookupCombo4Enter
  end
end
