...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, StdCtrls, DBCtrls, Mask, Buttons, ExtCtrls, DbTables,
  RxLookup, db, DBFilter;
...
procedure TfSitTributaria.FormClose(Sender: TObject;
...
begin
  if DM1.tUsuarioInsSitTrib.AsBoolean then
    begin
      ...
      DM1.tSitTributariaTipo.AsString    := 'N';
...
procedure TfSitTributaria.BitBtn6Click(Sender: TObject);
begin
  Close;
end;
...
begin
  ...
  if Length(Trim(DM1.tSitTributariaCodSit.AsString)) < 2 then
    begin
      ShowMessage('Código da situação tributária inválido!');
      exit;
    end;

  {if (Copy(DM1.tSitTributariaCodSit.AsString,1,1) <> '0') and
     (Copy(DM1.tSitTributariaCodSit.AsString,1,1) <> '1') and
     (Copy(DM1.tSitTributariaCodSit.AsString,1,1) <> '2') then
    begin
...
procedure TfSitTributaria.DBNavigator1BeforeAction(Sender: TObject;
  Button: TNavigateBtn);
begin
  DM1.tSitTributaria.IndexFieldNames := 'Codigo';
end;

procedure TfSitTributaria.DBMemo1KeyPress(Sender: TObject; var Key: Char);
begin
  Key := UpCase(Key);
end;

procedure TfSitTributaria.FormCloseQuery(Sender: TObject;
...
procedure TfSitTributaria.RxDBLookupCombo1Enter(Sender: TObject);
begin
  //DM1.tSitTributaria.IndexFieldNames := 'CodSit';
end;

procedure TfSitTributaria.RxDBLookupCombo1Exit(Sender: TObject);
begin
  //DM1.tSitTributaria.IndexFieldNames := 'Codigo';
end;

procedure TfSitTributaria.FormShow(Sender: TObject);
begin
