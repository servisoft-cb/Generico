...
uses
  Classes, SysUtils, Dialogs, SqlExpr, DmdDatabase, enviNFe_v400, DateUtils, Variants, uDM1, UNFe, UDMNotaFiscal;

  procedure prc_Gerar_XML_400(AStrings: TStrings);
...
//Letra W - Totais - Grupo de Valores Totais referentes ao ICMS
  ...
  NfeXML.InfNFe.Total.ICMSTot.VFCPUFDest   := '0.00';
  NfeXML.InfNFe.Total.ICMSTot.VICMSUFDest  := Replace(FormatFloat('0.00',DM1.tNotaFiscalVLR_ICMS_UF_DEST.AsFloat),',','.');
  NfeXML.InfNFe.Total.ICMSTot.VICMSUFRemet := Replace(FormatFloat('0.00',DM1.tNotaFiscalVLR_ICMS_UF_REMET.AsFloat),',','.');
  //4.00
  NfeXML.InfNFe.Total.ICMSTot.VFCP := '0.00';
  //****
  ...
  //4.00
  NfeXML.InfNFe.Total.ICMSTot.VFCPST    := '0.00';
  NfeXML.InfNFe.Total.ICMSTot.VFCPSTRet := '0.00';
  //*********
...
{* X  Informações do transporte da NF-e }
  ...
  begin
    ...
    begin
      ...
      detPag.IndPag := '0';
      if DM1.tTipoCobranca.Locate('ID',DM1.tNotaFiscalCodTipoCobr.AsInteger,([Locaseinsensitive])) then
        detPag.TPag := DM1.tTipoCobrancaCod_Imp_Fiscal.AsString
      else
        detPag.TPag := '';
      detPag.VPag := Replace(FormatFloat('0.00',DM1.tNotaFiscalVlrTotalDupl.AsFloat),',','.');
    end
    else
    begin
      fNFe.qTPag.Close;
      fNFe.qTPag.ParamByName('FILIAL').AsInteger := DM1.tNotaFiscalFilial.AsInteger;
      fNFe.qTPag.ParamByName('NumSeq').AsInteger := DM1.tNotaFiscalNumSeq.AsInteger;
      fNFe.qTPag.Open;
      fNFe.qTPag.First;
      while not fNFe.qTPag.Eof do
      begin
        detPag := NfeXML.InfNFe.Pag.DetPag.Add;
        //detPag.IndPag := '1';
        detPag.TPag := fNFe.qTPagCod_Imp_Fiscal.AsString;
        detPag.VPag :=  Replace(FormatFloat('0.00',fNFe.qTPagVLR_VENCIMENTO.AsFloat),',','.');
        fNFe.qTPag.Next;
      end;
    end;
    ...
    //begin
      vVlrTotalNota := StrToFloat(FormatFloat('0.00',DM1.tNotaFiscalVlrTotalNota.AsFloat - DM1.tNotaFiscalVlrTotalDupl.AsFloat));
...
{* H  Detalhamento de Produtos e Serviços da NF-e }
  ...
  if Trim(fNFe.mItensNFeCodBarra.AsString) = '' then
